...
export default function PropertyFilters({
  ...
  currentZone?: string;
  /** Selected slug of a sub-zone pill (e.g. "la-juanita"), only rendered
      when `subzones` is given — see PropertyListingPage for why this is a
      separate filter from the main zone one instead of nested options. */
  currentSubzone?: string;
  ...
  const ceil = Math.max(
    STEP,
    Math.ceil((priceBounds?.max ?? 5_000_000) / STEP) * STEP,
  );
  ...
  const pushWith = useMemo(
    () => (updates: Record<string, string | undefined>) => {
      ...
      // scroll: false — these updates (pills, sort, view) happen while the
      // user is already looking at the listing further down the page;
      // Next's default behavior of jumping back to the top on navigation
      // would otherwise throw them back up every time they pick one.
  ...
  function applyFilters(e: React.FormEvent<HTMLFormElement>) {
    ...
    pushWith({
      ...
      // subzona is applied live by the pills below, not by this form — see
      // their onClick — so it's deliberately left out of this submit.
      ...
      price_min:
        priceTouched && minPrice > floor ? String(minPrice) : undefined,
      price_max: priceTouched && maxPrice < ceil ? String(maxPrice) : undefined,
    });
  }

  const labelClass =
    "pb-1 text-[11px] font-medium uppercase tracking-wide text-brand-text/80";
  const fieldClass =
    "flex w-full min-w-0 flex-col gap-1 sm:w-auto sm:min-w-[130px] sm:flex-1";
  const selectClass =
  ...
  const pillClass = (active: boolean) =>
    `rounded-full border px-5 py-2 text-sm transition-colors ${
      active
        ...
        : "border-brand-text/30 bg-white text-brand-text hover:bg-brand-accent"
    }`;

  return (
    <div className={compact ? "" : "mb-10"}>
      <div
        className={`relative ${compact ? "" : "border-b border-brand-text/10 pb-8 sm:pb-14"}`}
      >
        <form
          onSubmit={applyFilters}
          className="flex w-full flex-wrap items-end gap-2.5"
        >
            ...
            <div className="flex w-full min-w-0 flex-col gap-1 sm:w-auto sm:min-w-[170px] sm:flex-[1.5]">
              ...
              <div className="relative h-5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setMinPrice(
                      Math.min(Number(e.target.value), maxPrice - STEP),
                    )
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setMaxPrice(
                      Math.max(Number(e.target.value), minPrice + STEP),
                    )
                  }
                  className="pointer-events-none absolute inset-0 h-5 w-full appearance-none bg-transparent [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border [&::-webkit-slider-thumb]:border-white [&::-webkit-slider-thumb]:bg-brand-primary"
                />
          ...
          {showSort ? (
            <label className={fieldClass}>
              ...
              <select
                value={currentSort ?? ""}
                onChange={(e) =>
                  pushWith({ sort: e.target.value || undefined })
                }
                className={selectClass}
              >
            ...
            <div className="flex w-full overflow-hidden rounded border border-brand-text/30 bg-white sm:inline-flex sm:w-auto">
              {VIEWS.map((v) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    pushWith({ view: v === "cards" ? undefined : v })
                  }
      ...
      {/* Sub-zone pills — below the Tarjetas/Lista/Mapa toggle, not a select,
          so picking one is a single click that filters live. */}
      {subzones && subzones.length > 0 ? (
        <div className="mt-10 flex flex-wrap gap-x-2.5 gap-y-3 sm:mt-12">
          <button
            type="button"
            onClick={() => pushWith({ subzona: undefined })}
            aria-pressed={!currentSubzone}
            className={pillClass(!currentSubzone)}
          >
            {t("allSubzones")}
          </button>
          {subzones.map((subzone) => (
            <button
              key={subzone.slug}
              type="button"
              onClick={() => pushWith({ subzona: subzone.slug })}
              aria-pressed={currentSubzone === subzone.slug}
              className={pillClass(currentSubzone === subzone.slug)}
            >
              {subzone.name}
            </button>
          ))}
